...
O `postinstall` copia o Bootstrap de `node_modules` para
`frontend/assets/vendor/bootstrap`. O frontend **não depende de CDN** — funciona
em rede local e offline.
...
| Variável | Descrição |
|---|---|
| `DB_USER`, `DB_PASSWORD`, `DB_NAME` | Acesso ao MySQL |
| `JWT_SECRET` | **Obrigatório trocar.** Gere um valor longo e aleatório |
| `ADMIN_EMAIL`, `ADMIN_PASSWORD` | Credenciais do administrador inicial |
| `CORS_ORIGINS` | Origens externas permitidas (o mesmo host é sempre liberado) |
...
### 4. Criar as tabelas e o administrador
...
```bash
node backend/database/seed.js --demo
```
...
**`Nao foi possivel conectar ao MySQL`** — confira se o serviço está rodando e
se `DB_USER`, `DB_PASSWORD` e `DB_NAME` no `.env` estão corretos.
