...
function buildFilters({ accountId, machineId, ownerId, status, from, to, userId }) {
...
async function list({ accountId, page = 1, pageSize = 20, machineId = null, ownerId = null, status = null,
  from = null, to = null, userId = null, orderDir = 'DESC' }) {
  const { whereSql, params } = buildFilters({ accountId, machineId, ownerId, status, from, to, userId });
...
async function findDetail(id, accountId) {
  return db.queryOne(
    `SELECT c.*, m.number AS machine_number, m.name AS machine_name, m.status AS machine_status,
      ...
      WHERE c.id = ? AND c.account_id = ? LIMIT 1`,
 ...
 */
async function findLastConfirmed(machineId, accountId, conn = null) {
  ...
  const sql = `SELECT id, collected_at, current_entry_value, current_exit_value,
                ...
                LIMIT 1`;
  if (conn) {
    const [rows] = await conn.execute(`${sql} FOR UPDATE`, [machineId, conta]);
    return rows.length ? rows[0] : null;
  }
  return db.queryOne(sql, [machineId, conta]);
}
...
async function create(conn, data) {
  const [result] = await conn.execute(
    `INSERT INTO collections
      (account_id, machine_id, owner_id, user_id,
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', ?, ?)`,
    [exigirConta(data.account_id), data.machine_id, data.owner_id, data.user_id,
...
async function listImages(collectionId, accountId) {
  return db.query(
    `SELECT ci.id, ci.collection_id, ci.file_path, ci.original_name, ci.mime_type,
            ci.size_bytes, ci.created_at
       FROM collection_images ci
       JOIN collections c ON c.id = ci.collection_id
      WHERE ci.collection_id = ? AND c.account_id = ?
      ORDER BY ci.id ASC`,
    [collectionId, exigirConta(accountId)]
  );
}

async function findImageById(imageId, accountId) {
  return db.queryOne(
    `SELECT ci.*, c.status AS collection_status, c.user_id AS collection_user_id
       FROM collection_images ci
       JOIN collections c ON c.id = ci.collection_id
      WHERE ci.id = ? AND c.account_id = ? LIMIT 1`,
    [imageId, exigirConta(accountId)]
  );
}

/** Cancela sem apagar fisicamente. */
async function cancel(conn, id, { userId, reason, cancelledAt, accountId }) {
  const [result] = await conn.execute(
    `UPDATE collections
        SET status = 'cancelled', cancelled_at = ?, cancelled_by = ?, cancellation_reason = ?
      WHERE id = ? AND account_id = ? AND status = 'confirmed'`,
    [cancelledAt, userId, reason, id, exigirConta(accountId)]
  );
  return result.affectedRows;
}

/** Trava a coleta dentro da transacao antes de cancelar. */
async function findForUpdate(conn, id, accountId) {
  const [rows] = await conn.execute(
    'SELECT * FROM collections WHERE id = ? AND account_id = ? FOR UPDATE',
    [id, exigirConta(accountId)]
  );
  return rows.length ? rows[0] : null;
}
...
async function totals({ accountId, machineId = null, ownerId = null, from = null, to = null }) {
  const { whereSql, params } = buildFilters({ accountId, machineId, ownerId, status: 'confirmed', from, to });
  return db.queryOne(
...
async function monthlySeries(accountId, months = 6) {
  return db.query(
    `SELECT DATE_FORMAT(c.collected_at, '%Y-%m') AS period,
            ...
            COALESCE(SUM(c.calculated_total_value), 0) AS total_value
       FROM collections c
      WHERE c.account_id = ? AND c.status = 'confirmed'
        AND c.collected_at >= DATE_SUB(DATE_FORMAT(CURDATE(), '%Y-%m-01'), INTERVAL ? MONTH)
      GROUP BY period
      ORDER BY period ASC`,
    [exigirConta(accountId), String(months - 1)]
  );
}
